...
def run(loaded, class_labels=None):
    ...
    # dataset[0] = list(dataset[0]) # Convert to list for easier manipulation
    for i in range(25):
        ...
        signals = signals[:, 0, :]  # reduce 3D to 2D
    ...
    print("Shape of signals (first subject):", dataset[0][0].shape)
     # Keep only the first channel (SpO2)

    memory = Memory(location='./tsfel_cache', verbose=0) 
    ...
    print("\nStarting Leave-One-Group-Out Cross-Validation with RandomForestClassifier...")
    ...
    for fold_num, (train_idx, test_idx) in enumerate(logo.split(features_all, y_all, groups_all)):
        ...
        # Handle potential all-NaN features *after* splitting (rare, but possible)
        if np.isnan(X_train).all(axis=0).any() or np.isnan(X_test).all(axis=0).any():
             print(f"Warning: Found feature columns with all NaNs in train/test split for fold {fold_num+1}. This might indicate issues upstream.")
             # Consider removing such features, or ensure imputation worked correctly
             X_train = np.nan_to_num(X_train) # Replace remaining NaNs with 0 if any slipped through
        ...
        # 4c) Train RandomForestClassifier
    ...
    print(f"\nMean accuracy (RandomForest): {mean_acc:.3f} ± {std_acc:.3f}")
    ...
    plt.title(f'Overall Confusion Matrix (RandomForest)\nMean Acc: {mean_acc:.3f}')
